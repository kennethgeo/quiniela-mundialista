...
# -----------------------------------------------------------------------------
# ACÁ VIVÍA UNA TERCERA COPIA DE LA FÓRMULA DEL TOTAL GLOBAL, y era la mala.
#
# `POST /api/matches/reconcile-totals` sumaba `champion_points + top_scorer_points`
# —SIN el asistidor— y sin deduplicar por partido ni por torneo, y con
# `?dry_run=false` ESCRIBÍA `users.total_points` con ese número. Es literalmente
# el fallo que CLAUDE.md tiene anotado como «los puntos de asistidor se
# olvidaron en una copia durante meses», vivo en un tercer sitio.
#
# Estaba MUERTO: el panel llama a `/api/admin/reconcile-totals`, que delega en
# `totales_desalineados()` + `recompute_user_total()` — o sea, en
# `user_total_calculado`, la única fórmula (migración 62). Nada en el repo, ni
# un workflow ni el frontend, apunta acá; comprobado.
#
# Medido el 22 sep 2026, antes de borrarlo: hoy no cambiaría ningún total, y no
# porque estuviera bien, sino porque todavía no se repartió ni un punto de
# asistidor (0 filas con `top_assist_points > 0`). El día que el admin reparta
# las globales de Bundestica, este botón le habría quitado 12 puntos a cada
# acertante sin decir nada.
#
# Se borra en vez de arreglarse: arreglarlo dejaría la fórmula escrita dos
# veces otra vez, que es el problema de fondo.
# -----------------------------------------------------------------------------
