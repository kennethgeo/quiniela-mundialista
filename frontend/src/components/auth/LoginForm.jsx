...
export default function LoginForm({ onToggle, onForgot, confirmationMessage }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      {confirmationMessage && (
        <div className="text-[12px] leading-snug rounded-lg p-3 bg-[#2ED3B7]/10 border border-[#2ED3B7]/25 text-[#2ED3B7]">{confirmationMessage}</div>
      )}
      {error && (
      ...
      <button type="button" onClick={onForgot} className="text-right text-[11px] font-semibold text-[#8A8A8A] hover:text-[#F3F1EA] transition-colors">
