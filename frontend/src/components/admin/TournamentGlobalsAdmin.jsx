...
export default function TournamentGlobalsAdmin() {
  ...
  useEffect(() => {
    supabase.from('tournaments').select('id, name, actual_champion, actual_top_scorer, predictions_locked').order('id')
      .then(({ data }) => { setTournaments(data || []); if (data?.[0]) selectT(data[0]) })
  }, [])

  const selectT = (t) => {
    setTid(t.id); setChampion(t.actual_champion || ''); setScorer(t.actual_top_scorer || ''); setLocked(!!t.predictions_locked)
  }
  ...
  const save = async () => {
    try {
      setBusy(true)
      await callAdmin('set-tournament-globals', { tournament_id: tid, actual_champion: champion, actual_top_scorer: scorer, predictions_locked: locked })
      setTournaments(prev => prev.map(t => t.id === tid ? { ...t, actual_champion: champion, actual_top_scorer: scorer, predictions_locked: locked } : t))
      flash('ok', 'Guardado.')
    } catch (e) { flash('error', e.message) } finally { setBusy(false) }
  }

  const calc = async () => {
    if (!confirm('¿Repartir los 12 pts de campeón/goleador de este torneo?')) return
    try {
      setBusy(true)
      await save()
