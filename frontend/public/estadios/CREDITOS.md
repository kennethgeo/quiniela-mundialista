# Créditos de las fotos de estadios

Todas salen de [Wikimedia Commons](https://commons.wikimedia.org). Se
redimensionaron y recomprimieron como JPEG; no se alteró el contenido de
ninguna.

**Al agregar una foto nueva, anotala acá.** Las licencias CC BY y CC BY-SA
exigen dar crédito al autor: sin esta lista estaríamos incumpliéndolas.

**Anotá también el AÑO de la foto y su categoría en Commons.** Sin eso no hay
manera de saber si una foto se quedó vieja salvo abrir las 48 y mirarlas: fue
justo lo que pasó cuando el dueño notó que el Bernabéu salía con las rampas de
hormigón anteriores a la remodelación. Con esos dos datos,
`scripts/revisar_estadios.py` le pregunta a Commons si hay algo más nuevo y lo
resuelve en un comando.

## Revisión de septiembre de 2026

Se comprobaron a ojo las que más habían cambiado:

| Foto | Estado |
|---|---|
| `bernabeu.jpg` | **Reemplazada.** La anterior era del estadio previo a la remodelación (rampas de hormigón a la vista). Ahora es el interior actual, con el anillo de pantallas y el techo. |
| `camp-nou.jpg` | **Vieja y sin reemplazo.** Es el graderío anterior al Espai Barça. En Commons no hay ninguna foto libre del estadio reconstruido: solo imágenes de la obra, que servirían menos que esta. Revisar más adelante. |
| `raiffeisen-linz.jpg` | **Correcta.** Es el Raiffeisen Arena inaugurado en 2023, no el Gugl viejo. |

Las 44 sedes de los torneos activos tienen foto; no falta ninguna.

## Dominio público — sin obligaciones

| Archivo | Estadio | Local | Autor |
|---|---|---|---|
| `morera-soto.jpg` | Alejandro Morera Soto | Alajuelense | Alonsogv |
| `carlos-ugalde.jpg` | Carlos Ugalde Álvarez | San Carlos e Inter | Alofaga |
| `perez-zeledon.jpg` | Municipal de Pérez Zeledón | Pérez Zeledón | Own Work |

## CC BY — hay que dar crédito, sin *share-alike*

| Archivo | Estadio | Local | Autor | Licencia |
|---|---|---|---|---|
| `carlos-alvarado.jpg` | Carlos Alvarado | Herediano | Periodista82 | CC BY 4.0 |
| `puente-piedra.jpg` | Puente Piedra | Sporting San José | Periodista82 | CC BY 4.0 |
| `rafael-bolanos.jpg` | Rafael Bolaños | Escorpiones Belén | Periodista82 | CC BY 4.0 |

## CC BY-SA — crédito y *share-alike*

| Archivo | Estadio | Local | Autor | Licencia |
|---|---|---|---|---|
| `saprissa.jpg` | Ricardo Saprissa Aymá | Saprissa | Jose David Murillo | CC BY-SA 4.0 |
| `fello-meza.jpg` | José Rafael "Fello" Meza | Cartaginés | Cartago CR | CC BY-SA 4.0 |
| `lito-perez.jpg` | Miguel Ángel "Lito" Pérez | Puntarenas | 7C7 | CC BY-SA 4.0 |
| `baltodano.jpg` | Edgardo Baltodano Briceño | Municipal Liberia | Judiego | CC BY-SA 3.0 |
| `colleya-fonseca.jpg` | José Joaquín "Colleya" Fonseca | Guadalupe FC | Alofaga | CC BY-SA 3.0 |
| `nacional.jpg` | Estadio Nacional | *(sanciones)* | MadriCR | CC BY-SA 4.0 |

El Nacional no es sede de nadie, pero se usa cuando a un club le sancionan el
estadio. Se deja puesto para ese caso.

## Champions League

Las **36 sedes** de la fase de liga, todas con foto.

### Dominio público / CC0 — sin obligaciones

| Archivo | Local | Autor | Licencia | Año | Categoría en Commons |
|---|---|---|---|---|---|
| `dragao.jpg` | FC Porto | JonnyJonny | Public domain |
| `fortuna-arena.jpg` | Slavia Praga | Tomas62 | CC0 |
| `metropolitano.jpg` | Atlético de Madrid | Zarateman | CC0 |
| `old-trafford.jpg` | Manchester United | Ank Kumar | CC0 |
| `olimpico-roma.jpg` | AS Roma | ? | Public domain |
| `philips.jpg` | PSV | Dmitriy Zhukov | Public domain |
| `rams-park.jpg` | Galatasaray | Qwl | Public domain |
| `signal-iduna.jpg` | Borussia Dortmund | Marvin Ronsdorf marvin_ronsd | CC0 |
| `sinigaglia.jpg` | Como | Fra Casa | CC0 |
| `stamford-bridge.jpg` | Shakhtar (local en Londres) | Dani- -152010 | CC0 |
| `villa-park.jpg` | Aston Villa | en:User:Miniluver04 | Public domain |
| `bernabeu.jpg` | Real Madrid | Badefa | CC0 | 2026 | `Category:Bernabéu Stadium` |

### CC BY — crédito, sin *share-alike*

| Archivo | Local | Autor | Licencia | Año | Categoría en Commons |
|---|---|---|---|---|---|
| `alvalade.jpg` | Sporting CP | Megutim | CC BY 4.0 |
| `bank-respublika.jpg` | Sabah FK | Nicat49 | CC BY 4.0 |
| `camp-nou.jpg` | Barcelona | Markus Unger from Vienna, Au | CC BY 2.0 |
| `cartuja.jpg` | Real Betis | Валерий Дед | CC BY 3.0 |
| `ceramica.jpg` | Villarreal | Emilio Leiras | CC BY 2.0 |
| `jan-breydel.jpg` | Club Brujas | Валерий Дед | CC BY 3.0 |
| `maradona.jpg` | Napoli | Gaetano Capaldo | CC BY 4.0 |
| `parc-des-princes.jpg` | PSG | Валерий Дед | CC BY 3.0 |
| `red-bull-leipzig.jpg` | RB Leipzig | Joeb07 | CC BY 3.0 |
| `tehelne-pole.jpg` | Slovan Bratislava | Mister No | CC BY 3.0 |
| `ulker.jpg` | Fenerbahçe | V&amp;A Dudush | CC BY 3.0 |
| `viking-stadion.jpg` | Viking FK | Pål Berge from Bryne, Norway | CC BY 2.0 |

### CC BY-SA — crédito y *share-alike*

| Archivo | Local | Autor | Licencia | Año | Categoría en Commons |
|---|---|---|---|---|---|
| `allianz-arena.jpg` | Bayern München | Diego Delso | CC BY-SA 3.0 |
| `anfield.jpg` | Liverpool | Ruaraidh Gillies | CC BY-SA 2.0 |
| `aspmyra.jpg` | Bodø/Glimt | Røed | CC BY-SA 2.5 |
| `bollaert.jpg` | Lens | JME91800 | CC BY-SA 4.0 |
| `de-kuip.jpg` | Feyenoord | Vincenzo.togni | CC BY-SA 4.0 |
| `emirates.jpg` | Arsenal | Arne Müseler | CC BY-SA 3.0 de |
| `etihad.jpg` | Manchester City | Ank kumar | CC BY-SA 4.0 |
| `mhparena.jpg` | VfB Stuttgart | Arne Müseler | CC BY-SA 3.0 de |
| `opap-arena.jpg` | AEK Atenas | Demian vi | CC BY-SA 4.0 |
| `pierre-mauroy.jpg` | Lille | Liondartois | CC BY-SA 4.0 |
| `raiffeisen-linz.jpg` | LASK | Matti Blume | CC BY-SA 4.0 |
| `san-siro.jpg` | Inter | Ank Kumar | CC BY-SA 4.0 |

### Cómo se consiguieron, por si hay que repetirlo

Buscar en Commons por texto y quedarse con el primer resultado **no funciona**.
Se intentó tres veces y las tres trajo cosas que el nombre del archivo no
delataba: «Anfield Road» era la calle, «Billetterie Stade Bollaert» la
boletería, «Ausblick zum Mercedes-Werk» unos viñedos, y también salieron una
placa en una pared, un parqueo y el techo de un pasillo.

Dos cosas que sí funcionaron:

1. **La imagen principal del artículo de Wikipedia** (`prop=pageimages`). Es
   la que los editores eligieron como LA foto del estadio, así que casi
   siempre es la buena. Resolvió 6 de las 10 difíciles — y el Parc des
   Princes solo aparece en la Wikipedia **francesa**, no en la inglesa.
2. **Bajar varios candidatos y mirarlos.** No hay atajo: el nombre del archivo
   no dice si se ve el estadio.

Y un sesgo a tener presente: **priorizar dominio público trae material
antiguo**, porque es lo que ya no tiene derechos. Así aparecieron el San Siro
en construcción (1926), una postal del Olímpico de 1959, el Alvalade viejo
—demolido en 2003— y una maqueta a escala del Bollaert.

## Nota sobre CC BY-SA

La cláusula *share-alike* pide que una obra derivada se publique con la misma
licencia. La tarjeta que arma la app incrusta estas fotos, así que se le puede
considerar derivada. Para un grupo de WhatsApp entre conocidos es
intrascendente; **si la app se abre al público conviene revisarlo**. Las CC BY
(sin *-SA*) no tienen ese problema: basta con esta lista.

Si se quisiera quedar solo con lo que no ata a nada, se borran las seis de la
última tabla y esos estadios pasan a usar el fondo sólido.
