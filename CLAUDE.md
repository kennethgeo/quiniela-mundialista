...
- **La migración 71 ya está aplicada** (7 sep 2026). Quitó el `UNIQUE (user_id)` que sobraba en `tournament_predictions` y que impedía tener predicciones globales en más de una quiniela: el error era `duplicate key value violates unique constraint "tournament_predictions_user_id_key"`. Comprobado leyendo `pg_constraint`: queda solo `UNIQUE (user_id, league_id)`, y las 15 predicciones existentes no se tocaron. **La 27, escrita para lo mismo, nunca se corrió** — de ahí venía el problema.
...
- **MEDIDO, Y ES PEOR QUE UN DUPLICADO: el recordatorio casi no sale.** El cron dice `*/15`, pero GitHub creó **44 corridas en 123 horas** (números 5→44, sin huecos, todas exitosas): una cada ~3 horas en vez de cada 15 minutos. Con una ventana de 15 min eso cubre el **8 % del tiempo**, así que **~92 % de los partidos no reciben ningún aviso**. No es que el endpoint falle: cuando corre, responde 200.
- **No es un límite general de GitHub.** En el mismo repo y periodo, `sync-live-scores.yml` con `*/5` acumula **1415** corridas (~98 % de lo esperado). Los dos workflows tienen la misma forma —igual bloque `concurrency`, sin condiciones—, así que la causa **no está identificada**. Lo que sí está medido es el efecto.
- El resumen diario también se corre: con `cron: "0 12 * * *"` se observó una ejecución a las **15:05 UTC**, o sea que el «aviso de las 6 am» llegó cerca de las **9 am** de Costa Rica.
- **La nota que había acá era falsa** y conviene no repetirla: decía que un atraso «corre la ventana con el reloj, nunca duplicado». Eso solo vale si TODAS las corridas se atrasan lo mismo. Una corrida de las 12:00 atrasada a 12:10 da `[12:55, 13:10)` y la de 12:15 puntual da `[13:00, 13:15)`: un saque a las 13:05 entra en las dos. En la práctica el duplicado **nunca pasó** (0 de 39 pares consecutivos separados por menos de 15 min), porque los huecos reales son de horas.
- **Conclusión: no se puede confiar en el cron de GitHub Actions para algo sensible a la hora.** Ningún arreglo dentro de la app —ventanas más anchas, anclar a la ranura del reloj— repara un disparador que no se dispara. La salida realista es un registro persistente de «a quién ya se le avisó» **más** un disparador fiable (por ejemplo colgarse del `*/5` de `sync-live-scores`, que sí corre) o un cron externo.
