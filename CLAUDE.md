...
### La liga tica no tiene alineaciones, y no es que tarden
Medido el 13 sep 2026 contra la API, no supuesto (lo reportó el dueño: «en la liga tica las alineaciones no están funcionando»):
- **Belén–Herediano, jornada 8, YA TERMINADO**: `rosters` con **0 jugadores**. La API `core` de ESPN devuelve `entries: []` para el mismo partido.
- **Inter de San Carlos–Puntarenas, misma fecha**: 16 y 12 nombres **sin dorsal, sin formación y todos con posición «SUB»**. No es un once anunciado: es la lista de quienes aparecieron en algún evento del partido.
- **Contraste con la misma petición**: `esp.1`, `eng.1` y `uefa.champions` devuelven **11 titulares por equipo con formación**. O sea que el recorte funciona; lo que no hay es dato.
- **Y ahí había un fallo nuestro**: `_alineaciones` descartaba con `if not titulares`, así que esos 10 y 8 «titulares» **pasaban el filtro y se dibujaban en la cancha** como si el equipo los hubiera anunciado. Ahora se exigen **once** (`TITULARES_DE_UN_ONCE`). Enseñar una alineación inventada es peor que no enseñar ninguna: alguien predice con eso.
- **«Todavía no» y «nunca» no se dicen con la misma frase.** El panel prometía «en cuanto salgan aparecen acá» en un torneo donde no van a salir — alguien se queda recargando hasta el saque. El backend manda `fuente_con_alineaciones` (de `LIGAS_SIN_ALINEACIONES`, hoy `{'crc.1'}`) y la pantalla elige el texto. **Manda el dato, no la lista**: si ESPN empieza a publicarlas, la alineación se dibuja igual. Un `None` —una respuesta guardada antes de que el campo existiera— se lee como «no sabemos» y dice lo de siempre: un hueco no se convierte en una afirmación.
- **UNAFUT no sirve como fuente**: unafut.com es un CMS de noticias (pixeles.club/geniuswebs) **sin API**; el sitemap solo trae notas y páginas de club, `/calendario` no enlaza a ninguna ficha de partido y no hay alineaciones en ninguna parte. Comprobado, no supuesto.
- **No sirve mirar solo un partido para concluir «esta liga no tiene»**: el primero que consulté (Belén–Herediano) daba 0 y parecía cerrado; el segundo de la misma jornada dio 28 nombres y cambió el diagnóstico —de «no hay nada» a «hay basura que estábamos dibujando»—, que era el fallo de verdad.
